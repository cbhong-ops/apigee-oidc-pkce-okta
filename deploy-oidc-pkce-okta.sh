...
apigeecli products get --name oidc-pkce-okta-product --org "$APIGEE_ORG" --default-token &>/dev/null
if [ $? -ne 0 ]; then
  echo "Creating API Product: oidc-pkce-okta-product"
  apigeecli products create --name oidc-pkce-okta-product --display-name "OIDC PKCE Okta Product" --envs "$APIGEE_ENV" --proxies "$PROXY_NAME" --approval auto --org "$APIGEE_ORG" --default-token
else
  echo "API Product oidc-pkce-okta-product already exists."
fi
...
if [ $? -ne 0 ]; then
  ...
  apigeecli developers create --user oidc-pkce-dev --email "$DEV_EMAIL" --first OIDC --last Developer --org "$APIGEE_ORG" --default-token
else
...
if [ $? -ne 0 ]; then
  echo "Creating Developer App: $APP_NAME and subscribing to oidc-pkce-okta-product..."
  apigeecli apps create --name "$APP_NAME" --email "$DEV_EMAIL" --prods oidc-pkce-okta-product --org "$APIGEE_ORG" --default-token --disable-check
else
...
CLIENT_ID=$(echo "$APP_INFO" | jq -r 'if type == "array" then .[0] else . end | .credentials[] | select(.apiProducts[].apiproduct=="oidc-pkce-okta-product") | .consumerKey')
CLIENT_SECRET=$(echo "$APP_INFO" | jq -r 'if type == "array" then .[0] else . end | .credentials[] | select(.apiProducts[].apiproduct=="oidc-pkce-okta-product") | .consumerSecret')
